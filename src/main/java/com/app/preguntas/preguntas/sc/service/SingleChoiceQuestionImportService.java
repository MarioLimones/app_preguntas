...
@Service
public class SingleChoiceQuestionImportService {

    private final SingleChoiceQuestionService service;
    private final ObjectMapper objectMapper;

    public SingleChoiceQuestionImportService(SingleChoiceQuestionService service, ObjectMapper objectMapper) {
        this.service = service;
        this.objectMapper = objectMapper;
    }

    public SingleChoiceQuestionImportResult importFile(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            return new SingleChoiceQuestionImportResult(0, Collections.emptyList(),
    ...
    private SingleChoiceQuestionImportResult importJson(MultipartFile file) throws IOException {
        List<SingleChoiceQuestion> input = objectMapper.readValue(
            file.getInputStream(),
            new TypeReference<List<SingleChoiceQuestion>>() {}
        );
        return createQuestions(input);
    }

    private SingleChoiceQuestionImportResult importCsv(MultipartFile file) throws IOException {
        List<SingleChoiceQuestion> created = new ArrayList<>();
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                total++;
                SingleChoiceQuestion question = parseCsvQuestion(values, header, lineNumber, errors);
                if (question == null) {
                    continue;
                }
        ...
        return new SingleChoiceQuestionImportResult(total, created, errors);
    }

    private SingleChoiceQuestionImportResult createQuestions(List<SingleChoiceQuestion> input) {
        List<SingleChoiceQuestion> created = new ArrayList<>();
        List<String> errors = new ArrayList<>();
        int index = 0;
        if (input == null) {
            return new SingleChoiceQuestionImportResult(0, created, List.of("El archivo JSON debe contener una lista."));
        }
        for (SingleChoiceQuestion question : input) {
            index++;
            SingleChoiceQuestion normalized = normalize(question);
            String error = SingleChoiceQuestionValidator.validate(normalized);
            if (error != null) {
        ...
        return new SingleChoiceQuestionImportResult(input.size(), created, errors);
    }

    private SingleChoiceQuestion normalize(SingleChoiceQuestion question) {
        if (question == null) {
            return new SingleChoiceQuestion();
        }
        SingleChoiceQuestion normalized = new SingleChoiceQuestion();
    ...
    private SingleChoiceQuestion parseCsvQuestion(List<String> values,
