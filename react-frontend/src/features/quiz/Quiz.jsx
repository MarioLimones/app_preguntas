...
const Quiz = () => {
    ...
    const fetchQuestions = async () => {
        setLoading(true);
        try {
            const storageKey = `quiz_queue_${type}`;
            let queue = [];
            ...
            // Fetch all questions to check for statements and IDs
            const response = await api.get(`/${type}/questions`);
            const allFetched = response.data || [];

            // DEDUP by statement: only one ID per unique statement
            ...
            const uniqueIds = uniqueQuestions.map(q => q.id);

            // Filter queue to only contain IDs that still exist in our unique list
            queue = queue.filter(id => uniqueIds.includes(id));

            if (queue.length < QUESTIONS_PER_QUIZ) {
    ...
    if (questions.length === 0) return (
        <div className="text-center p-12 bg-white rounded-3xl shadow-xl mt-12">
            ...
            <button onClick={() => navigate('/')} className="mt-6 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold">Volver al inicio</button>
        </div>
    );
            ...
            <div className="max-w-2xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                <div className="space-y-4 mb-10">
                    {questions.map((q, idx) => {
                        ...
                        return (
                            <div key={q.id} className={`p-6 rounded-2xl border-2 transition-colors ${isCorrect ? 'bg-emerald-50/50 border-emerald-100' : 'bg-rose-50/50 border-rose-100'}`}>
        ...
        <div className="max-w-3xl mx-auto animate-in fade-in duration-500">
            <div className="mb-6 flex items-center justify-between">
                <div className="bg-white px-4 py-2 rounded-2xl shadow-sm border border-gray-100 text-sm font-bold text-gray-500">
                    Pregunta <span className="text-indigo-600">{currentIndex + 1}</span> de {questions.length}
                </div>
                <div className="bg-indigo-50 px-4 py-2 rounded-2xl text-xs font-black text-indigo-600 uppercase tracking-widest">
                    {type === 'vf' ? 'Verdadero o Falso' : type === 'sc' ? 'Selección Única' : 'Selección Múltiple'}
                </div>
            </div>
