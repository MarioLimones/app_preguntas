package com.app.quiz.features.sc.model;

public class SingleChoiceReviewItem {

    private final SingleChoiceQuestion question;
    private final Integer userAnswer;

    public SingleChoiceReviewItem(SingleChoiceQuestion question, Integer userAnswer) {
        this.question = question;
        this.userAnswer = userAnswer;
    }

    public SingleChoiceQuestion getQuestion() {
        return question;
    }

    public Integer getUserAnswer() {
        return userAnswer;
    }

    public boolean isCorrect() {
        if (question == null || userAnswer == null || question.getCorrectIndex() == null) {
            return false;
        }
        return userAnswer.equals(question.getCorrectIndex());
    }
}



